// Copyright (c) 2013-2014 The Bitcoin Core and Deuscoin Core developers
...
BOOST_AUTO_TEST_CASE(rpc_wallet)
{
	...
	CDeuscoinAddress demoAddress = CDeuscoinAddress(CTxDestination(demoPubkey.GetID()));
	Value retValue;
	...
	BOOST_CHECK(CDeuscoinAddress(retValue.get_str()).Get() == demoAddress.Get());
	...
	BOOST_CHECK(CDeuscoinAddress(arr[0].get_str()).Get() == demoAddress.Get());
