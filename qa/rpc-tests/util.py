...
def check_json_precision():
    """Make sure json library being used does not lose precision converting DEUS values"""
...
def initialize_datadir(dir, n):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    with open(os.path.join(datadir, "deuscoin.conf"), 'w') as f:
        f.write("regtest=1\n");
        f.write("rpcuser=rt\n");
...
def initialize_chain(test_dir):
    ...
    4 wallets.
    deuscoind and deuscoin-cli must be in search path.
    """

    if not os.path.isdir(os.path.join("cache", "node0")):
        devnull = open("/dev/null", "w+")
        # Create cache directories, run deuscoinds:
        for i in range(4):
            datadir=initialize_datadir("cache", i)
            args = [ "deuscoind", "-keypool=1", "-datadir="+datadir, "-discover=0" ]
            if i > 0:
                args.append("-connect=127.0.0.1:"+str(p2p_port(0)))
            deuscoind_processes[i] = subprocess.Popen(args)
            subprocess.check_call([ "deuscoin-cli", "-datadir="+datadir,
    ...
    for i in range(4):
        ...
        initialize_datadir(test_dir, i) # Overwrite port/rpcport in deuscoin.conf
...
def start_node(i, dir, extra_args=None, rpchost=None):
    """
    Start a deuscoind and return RPC connection to it
    """
    datadir = os.path.join(dir, "node"+str(i))
    args = [ "deuscoind", "-datadir="+datadir, "-keypool=1", "-discover=0" ]
...
def start_nodes(num_nodes, dir, extra_args=None, rpchost=None):
    """
    Start multiple deuscoinds, return RPC connections to them
    """
...
def stop_node(node, i):
    node.stop()
    deuscoind_processes[i].wait()
    del deuscoind_processes[i]
...
def wait_deuscoinds():
    # Wait for all deuscoinds to cleanly exit
    for deuscoind in deuscoind_processes.values():
        deuscoind.wait()
    deuscoind_processes.clear()
